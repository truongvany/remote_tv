...
class AdbProtocol : TVProtocol {
    ...
    override suspend fun sendCommand(command: String): Boolean = withContext(Dispatchers.IO) {
        ...
        if (command.startsWith("TEXT:")) {
            ...
            return@withContext sendTextCommand(normalized)
        }
    ...
    private fun sendTextCommand(text: String): Boolean {
        val hasNonAscii = text.any { it.code > 0x7F }
        return if (hasNonAscii) {
            InAppDiagnostics.info(TAG, "ADB TEXT non-ASCII detected, using clipboard+paste fallback")
            sendTextViaClipboard(text)
        } else {
            val escaped = escapeForAdbInputText(text)
            sendShellCommand("input text $escaped\n").also { sent ->
                InAppDiagnostics.info(TAG, "ADB TEXT ascii path sent=$sent length=${text.length}")
            }
        }
    }

    private fun sendTextViaClipboard(text: String): Boolean {
        val escaped = escapeForSingleQuotedShellArg(text)
        val cmd = "cmd clipboard set text '$escaped' && input keyevent 279\n"
        return sendShellCommand(cmd).also { sent ->
            if (sent) {
                InAppDiagnostics.info(TAG, "ADB TEXT clipboard+paste command sent")
            } else {
                InAppDiagnostics.warn(TAG, "ADB TEXT clipboard+paste send failed")
            }
        }
    }
    ...
    private fun escapeForSingleQuotedShellArg(text: String): String {
        return text.replace("'", "'\\''")
    }
